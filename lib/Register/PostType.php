...
class PostType
{
    ...
    public function setLabels($labels, $singularName = false)
    {
        ...
        if (is_string($singularName) && is_string($labels)) {
            $pluralName           = $labels;
            $pluralNameLC         = \LengthOfRope\Treehouse\Utils\StringParser::factory($pluralName)->toLower();
            $singularNameLC       = \LengthOfRope\Treehouse\Utils\StringParser::factory($singularName)->toLower();
